...
var finishExcel = require('../../scripts/finishexcel');
...
exports.index = function(req, res){
  ...
  new Promise((resolve, reject) =>{
    ...
    res.render('dashboard/finish/finish', {customers: customers, colors: colors, allcustomers: allcustomers, role: res.role});    
  })
}
...
exports.add_finish = function(req, res){
  Finish.add(req.body, function(err, result){
    if(err){
      res.json({isSuccess: false});
    }else{
...
exports.update_finish = function(req, res){
  Finish.update(req.body, function(err, result){
    if(err){
      res.json({isSuccess: false});
    }else{
...
exports.list_finish = function(req, res){
  var finishlist = [], orderlist = [];
  var p1 = new Promise((resolve, reject) => {
    Finish.list(req.body, function(err, result){
      if(err){
        finishlist = [];
        reject();
      }else{
  ...
  .then(() => {
    ...
    res.json({isSuccess: true, finish: finishlist, order: order});
  })
}

exports.delete_finish = function(req, res) {
  Finish.remove(req.body, function(err, result){
    if(err){
      res.json({isSuccess: false});
    }else{
      res.json({isSuccess: true});
...
exports.upload_finish = function(req, res) {
        ...
        fs.writeFile(new_path, data, function(err) {
          fs.unlink(old_path, function(err) {
            if (err) {
              ...
              finishExcel(new_path, fields.orderid, function(err, result){
                if(err){
                  res.json({isSuccess: false});
                }else{
