  ...
  new Promise((resolve, reject) => {
    ...
    const add = function(index){
      ...
      if(tmp.length == 0){
        ...
        var data = {
          po: tmp[1], order: tmp[0], color: tmp[2], washdate: myDate.getFullYear() + "-" + (myDate.getMonth() + 1 < 10? '0' + (myDate.getMonth() + 1) : myDate.getMonth() + 1 ) + "-" + (myDate.getDate()<10?'0'+(myDate.getDate()): (myDate.getDate())),
        ...
        WashReturn.add(data, function(err, result){
          if(err){
            fails.push(index);
          } else {
            if(index < exceldata.length-1){
