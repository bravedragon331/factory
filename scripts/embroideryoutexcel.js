...
var embroideryExcel = function(path, orderid, callback){
  ...
  new Promise((resolve, reject) => {
    ...
    const add = function(index){
      ...
      if(tmp.length == 0){
        ...
        var data = {
          po: tmp[1], order: tmp[0], color: tmp[2], embroiderydate: myDate.getFullYear() + "-" + (myDate.getMonth() + 1 < 10? '0' + (myDate.getMonth() + 1) : myDate.getMonth() + 1 ) + "-" + (myDate.getDate()<10?'0'+(myDate.getDate()): (myDate.getDate())),
        ...
        EmbroideryOut.add(data, function(err, result){
          if(err){
            fails.push(index);
          } else {
            if(index < exceldata.length-1){
...
module.exports = embroideryExcel;